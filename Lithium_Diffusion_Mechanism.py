    ...
    with open(input_file,'r') as r:
        for line in r:
            ...
            if l[0]=='t' and float(l[2])!=0:
                break 
    n_atoms=0
    for i in np.arange(len(atom_index)):
        for j in np.arange(len(atom_index[i])):
...
def Change_Index():
    x=[]
    y=[]
    for i in np.arange(len(in_ref)):
        x.append([])
        y.append([])
        nums_used=[]
        for j in np.arange(len(observable)):
            x[i].append([])
            y[i].append([])
            nums_used.append([])
...
def Plot_Results(x,y):
    ...
    if num_subplots>1:
        ...
        for i in np.arange(len(x)):
            if len(y[i])==0:
                continue
            for j in np.arange(len(atom_id)):
            ...
            for j in np.arange(len(del_mol)):
