...
#   molecules of the system with respect to a observble molecule.    #
#                                                                    #
...
observable=Type_of_Interaction()
...
poly=''
if is_poly=='y':
    for i in np.arange(len(molecule_id)):
        print('%s = %i' %(molecule_id[i],i))
    print('\n')
    which_poly=int(input('Which one?: '))
    poly=molecule_id[which_poly]
...
for i in range(len(coordination)):
    ...
    for j in range(len(coordination[i])):
        if molecule_id[i]==poly:
            saturation[i].append(coordination[i][j]/len(atom_index[i][0]))
        else:
