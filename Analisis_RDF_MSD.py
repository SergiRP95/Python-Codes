...
# Function that reads the MSD data.
# Time in ps to ns.
# MSD in nm^2 to A^2.
...
# Functions that reads the RDF data.
# Radius in pm to A.
def lect_rdf(input_file,radius=False):
	file=open(input_file+'.csv')
	data=[]
	for line in file:
		l=line.split()
...
# To identify this region, we create the straight line tangent to each point 
# in log-log scale and find the section in which the data deviate less from the
# straight line.
# D in cm^2/s.
def compute_D(msd):
	...
	for i in range(len(msd_corr)):
	    lower_limit.append(msd_corr[i]-margen_error)
	    upper_limit.append(msd_corr[i]+margen_error)
	...
	x_ajuste=region_corr[pos_max]
	y_ajuste=[]
	for i in x_ajuste:
		y_ajuste.append(msd_corr[t_corr.index(i)])   
	coef,V=np.polyfit(x_ajuste,y_ajuste,1,cov=True)
	print('Compute D')
	return(coef[0]*1e-7/6,np.sqrt(V[0][0])*1e-7/6)


# Function that computes the mean value and error.
def make_mean(data):
	mean=[]
	error=[]
